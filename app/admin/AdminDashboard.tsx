...
export default function AdminDashboard() {
  ...
  const updateConfig = (side: GuestSide, field: "venueName" | "address" | "mapUrl" | "musicUrl", value: string) => setConfigs((current) => ({ ...current, [side]: { ...current[side], [field]: value } }));
  ...
  const saveConfig = async (event: FormEvent<HTMLFormElement>, side: GuestSide) => {
    ...
    const response = await fetch("/api/admin/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ family: side, venueName: config.venueName, address: config.address, mapUrl: config.mapUrl, musicUrl: config.musicUrl, eventDate: config.date.iso }) });
