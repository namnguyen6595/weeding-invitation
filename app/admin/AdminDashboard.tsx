...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (response.status === 401) { setAuthenticated(false); return; }
      const payload = (await response.json()) as { rsvps?: Rsvp[] };
      if (!cancelled) { setRsvps(payload.rsvps ?? []); setAuthenticated(true); void loadConfigs(); }
    };
    void checkSession();
  ...
  const submitLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setPassword(""); await loadRsvps(); await loadConfigs(); setLoading(false);
  };

  const updateConfig = (side: GuestSide, field: "venueName" | "address" | "mapUrl", value: string) => setConfigs((current) => ({ ...current, [side]: { ...current[side], [field]: value } }));
  ...
  const saveConfig = async (event: FormEvent<HTMLFormElement>, side: GuestSide) => {
    event.preventDefault(); setSavingConfig(side); setConfigMessage("");
    const config = configs[side];
    const response = await fetch("/api/admin/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ family: side, venueName: config.venueName, address: config.address, mapUrl: config.mapUrl, eventDate: config.date.iso }) });
  ...
  return <main className="admin-page"><header className="admin-header"><div><p className="admin-eyebrow">Wedding dashboard</p><h1>Nam &amp; Mai</h1></div><button className="admin-logout" type="button" onClick={logout}>Đăng xuất</button></header><MusicSettingsForm /><section className="admin-config-editor"><div className="admin-section-heading"><div><p className="admin-eyebrow">Wedding settings</p><h2>Cấu hình thông tin hai bên</h2></div>{configMessage && <span className="admin-save-message">{configMessage}</span>}</div><div className="admin-config-grid">{(["groom", "bride"] as GuestSide[]).map((side) => { const config = configs[side]; return <form className="admin-config-card" key={side} onSubmit={(event) => saveConfig(event, side)}><span>{config.shortLabel}</span><label>Tên địa điểm<input value={config.venueName} onChange={(event) => updateConfig(side, "venueName", event.target.value)} /></label><label>Địa chỉ<input value={config.address} onChange={(event) => updateConfig(side, "address", event.target.value)} /></label><label>Google Maps URL<input type="url" value={config.mapUrl} onChange={(event) => updateConfig(side, "mapUrl", event.target.value)} placeholder="https://maps.google.com/..." /></label><label>Ngày và giờ<input type="datetime-local" value={config.date.iso.slice(0, 16)} onChange={(event) => updateConfigDate(side, event.target.value)} /></label><button type="submit" disabled={savingConfig === side}>{savingConfig === side ? "Đang lưu…" : "Lưu cấu hình"}</button></form>; })}</div></section><section className="admin-config-grid">{(["groom", "bride"] as GuestSide[]).map((side) => { const config = configs[side]; return <article className="admin-config-card" key={side}><span>{config.shortLabel}</span><h2>{config.venueName || "Chưa cập nhật tên địa điểm"}</h2><p>{config.address || "Chưa cập nhật địa chỉ"}</p><strong>{config.date.display}</strong><a href={config.mapUrl || undefined} target="_blank" rel="noreferrer">{config.mapUrl ? "Mở bản đồ ↗" : "Bản đồ chưa cập nhật"}</a></article>; })}</section><section className="admin-summary"><article><span>Tổng phản hồi</span><strong>{rsvps.length}</strong></article><article><span>Sẽ tham dự</span><strong>{attending.length}</strong></article><article><span>Tổng số khách</span><strong>{guestCount}</strong></article><article><span>Lời chúc</span><strong>{rsvps.filter((rsvp) => rsvp.message.trim()).length}</strong></article></section><section className="admin-responses"><div className="admin-section-heading"><div><p className="admin-eyebrow">Guest responses</p><h2>Danh sách khách mời</h2></div><select value={filter} onChange={(event) => setFilter(event.target.value as "all" | GuestSide)}><option value="all">Tất cả</option><option value="groom">Nhà trai</option><option value="bride">Nhà gái</option></select></div>{visibleRsvps.length === 0 ? <p className="admin-empty">Chưa có phản hồi nào.</p> : <div className="admin-table-wrap"><table><thead><tr><th>Khách mời</th><th>Phía</th><th>Phản hồi</th><th>Số người</th><th>Lời chúc</th><th>Thời gian</th></tr></thead><tbody>{visibleRsvps.map((rsvp) => <tr key={rsvp.id}><td>{rsvp.guestName}</td><td>{rsvp.family === "groom" ? "Nhà trai" : "Nhà gái"}</td><td><span className={`admin-status ${rsvp.attendance}`}>{rsvp.attendance === "yes" ? "Tham dự" : "Không tham dự"}</span></td><td>{rsvp.guestCount}</td><td className="admin-message">{rsvp.message || "—"}</td><td>{new Date(rsvp.createdAt).toLocaleString("vi-VN")}</td></tr>)}</tbody></table></div>}</section></main>;
}
